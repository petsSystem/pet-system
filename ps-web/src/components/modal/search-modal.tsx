...
interface SearchModalProps<T extends HasName> {
  isOpen: boolean;
  setIsOpen: (value: boolean) => void;
  data: T[];
  onSearch?: (searchValue: string) => void;
  minLength?: number;
  onItemClick?: (item: T) => void; // Adicionando a propriedade onItemClick
}
...
export default function SearchModal<T extends HasName>({
  isOpen,
  setIsOpen,
  data,
  onSearch,
  minLength = 0,
  onItemClick, // Recebendo a função onItemClick
}: SearchModalProps<T>) {
      ...
      <Dialog as="div" onClose={() => setIsOpen(false)}>
        ...
        >
          <Dialog.Panel className="bg-white dark:bg-slate-800 border border-transparent dark:border-slate-700 overflow-auto max-w-2xl w-full max-h-full rounded shadow-lg">
            ...
            <form className="border-b border-slate-200 dark:border-slate-700"></form>
              ...
              <div className="mb-3 last:mb-0">
                ...
                <ul className="text-sm">
                  {data.map((item, index) => (
                    <li key={index}>
                      <button
                        className="flex w-full items-center p-2 text-slate-800 dark:text-slate-100 hover:text-white hover:bg-primary-500 focus-within:text-white focus-within:bg-primary-500 outline-none rounded group"
                        onClick={() => handleItemClick(item)} // Lidando com o clique no item da lista
                      >
                        <svg
