...
export default function ProductsTableItem({
  ...
  const { totalColor, statusColor, typeIcon } = ProductsProperties();

  const formattedAmount = formatNumberToCurrency(product.amount);

  return (
    <tr>
      <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
        <div className={`font-medium  ${totalColor(product.active)}`}>
          {product.name}
        </div>
      </td>
      <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
